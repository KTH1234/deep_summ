...
class HierarchicalDataset(ONMTDatasetBase):
    """ Dataset for data_type=='text'
        ...
        Args:
            ...
            context_delimiter_char (str) : context delimiter character or string
            remove_delimiter (bool) : remove context delimiter in data
    """
    def __init__(self, fields, src_examples_iter, tgt_examples_iter, context_delimiter_char, remove_delimiter,
        ...
        print("Hierarchical dataset line:49 context_delimiter_char", context_delimiter_char)
        print("Hierarchical dataset line:50 remove_delimiter", remove_delimiter)
...
#             print("TextDataset, line:50", self._join_dicts(src, tgt))
        ...
        examples_iter = self.make_context_mask(examples_iter)         
...
#             input()
    ...
    def get_fields(n_src_features, n_tgt_features):
        ...
        def align_context_mask(data, vocab, is_train):
            tgt_size = max([t.size(0) for t in data])
            alignment = -1 * torch.ones(tgt_size, len(data)).long()
            for i, mask in enumerate(data):
                alignment[:mask.size(0), i] = mask
             
            return alignment
        
        # for hierarchical encoder
        fields["context_mask"] = torchtext.data.Field(
            use_vocab=False, tensor_type=torch.LongTensor,
            postprocessing=align_context_mask, sequential=False)
        
        fields["context_lengthes"] = torchtext.data.Field(
            use_vocab=False, tensor_type=torch.LongTensor,
            sequential=False)        
    ...
    def get_num_features(corpus_file, side):
        ...
        with codecs.open(corpus_file, "r", "utf-8") as cf:
            ...
            _, _, num_feats = HierarchicalDataset.extract_text_features(f_line)
...
#                 input()  
    ...
    # make context_mask for hierarchical encoding
    def make_context_mask(self, examples_iter):
        for example in examples_iter:
            src = example["src"]
            ...
            for context_idx, delimiter_index in enumerate(delimiter_indexes):
                if prev_idx == 0:
                    prev_idx = delimiter_index+1
                    continue
                context_mask[prev_idx:delimiter_index+1] = [context_idx] * (delimiter_index - prev_idx + 1)
                prev_idx = delimiter_index+1
                
            # need test
            if self.remove_delimiter:
                example["src"] = [ word for word in example["src"] if word != self.context_delimiter_char ]
                for index in reversed(delimiter_indexes):
                    del context_mask[index]
                assert len(example["src"])  == len(context_mask)
                
            example["context_mask"] = torch.LongTensor(context_mask)
            example["context_lengthes"] = len(delimiter_indexes)
            
#             for word, id in zip(example["src"], example["context_mask"]):
#                 print(word, id)
#             print(example["context_lengths"])
#             input()
